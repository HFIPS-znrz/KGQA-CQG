...
def validate_v2(data_path, device, model, dataloader, entity2idx, model_name, writeCandidatesToFile=False):
    ...
    for i in tqdm(range(len(data))):
        ...
        pred_, idx1 = new_scores.topk(k=3, dim=0, largest=True, sorted=True)
        # print('p',pred_)
        pred_ans = []
        pred_text = []
        for i in range(3):
            pred_text.append(idx2entity[idx1[i].item()])
            pred_ans.append(idx1[i].item())
        # pred_ans = torch.argmax(new_scores).item()
        ...
        #     print('Answer not in candidates')
            ...
            # exit(0)
        all_pred_ans.append(pred_text)

        t = []
        for a in ans:
            t.append(idx2entity[a])
        all_correct_ans.append(t)
    ...
    if writeCandidatesToFile:
        ...
        pickle.dump(candidates_with_scores, open('corn_scores_full_kg.pkl', 'wb'))
    ...
    # print(answers)
    accuracy = total_correct / (total_correct + num_incorrect)
    ...
    def precision_recall_at_k(actual, predicted, k):
        ...
        for i in range(len(actual)):
            tp = len(set(predicted[i][:k]) & set(actual[i]))
            precision = tp / k
    ...
    def macro_precision_recall_f1_at_k(actual, predicted, k):
        precision_list, recall_list = precision_recall_at_k(actual, predicted, k)
    ...
    def micro_precision_recall_f1_at_k(actual, predicted, k):
        tp = sum(len(set(predicted[i][:k]) & set(actual[i])) for i in range(len(actual)))
        fp = sum(len(set(predicted[i][:k])) - len(set(predicted[i][:k]) & set(actual[i])) for i in range(len(actual)))
        fn = sum(len(actual[i]) - len(set(predicted[i][:k]) & set(actual[i])) for i in range(len(actual)))
    ...
    def average_precision(actual, predicted):
        ...
        for i, p in enumerate(predicted):
            if p in actual and p not in predicted[:i]:
                num_hits += 1.0
                avg_precision += num_hits / (i + 1.0)
        if not actual:
            return 0.0
        return avg_precision / len(actual)

    def mean_average_precision(actual_list, predicted_list):
        return sum(average_precision(a, p) for a, p in zip(actual_list, predicted_list)) / len(actual_list)

    # 示例数据
    actual = [tuple(a) for a in all_correct_ans]
    predicted = [tuple(a) for a in all_pred_ans]
    print(all_pred_ans)
    print(all_correct_ans)

    # 计算指标
    k = 1
    # print('act',actual)
    # print('pre', predicted)
    macro_precision, macro_recall, macro_f1 = macro_precision_recall_f1_at_k(actual, predicted, k)
    micro_precision, micro_recall, micro_f1 = micro_precision_recall_f1_at_k(actual, predicted, k)

    map_score = mean_average_precision(actual, predicted)

    print("Macro Precision@{}:".format(k), macro_precision)
    print("Macro Recall@{}:".format(k), macro_recall)
    print("Macro F1@{}:".format(k), macro_f1)
    print("Micro Precision@{}:".format(k), micro_precision)
    print("Micro Recall@{}:".format(k), micro_recall)
    print("Micro F1@{}:".format(k), micro_f1)
    print("MAP:", map_score)
...
def getEntityEmbeddings(kge_model, hops):
    e = {}
    entity_dict = '../../pretrained_models/embeddings/entity_ids.del'
    if 'half' in hops:
        entity_dict = '../../pretrained_models/embeddings/entity_ids.del'
...
data_path = '../../data/corn2/qa_train.txt'
valid_data_path = '../../data/corn2/qa_dev.txt'
test_data_path = '../../data/corn2/qa_test.txt'
