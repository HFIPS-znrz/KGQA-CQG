...
def build_encoder(opt, embeddings, embeddings1):
    """
    ...
    if opt.encoder_type == "transformer":
        with open("/home/dell/zjx/MSParS_me/MSParS_V2.0/code/generation/OpenNMT-py-master-our-model/onmt/args.yaml", encoding='utf-8') as f:
            configs = yaml.load(f.read(), Loader=yaml.FullLoader)
            configs = DefaultMunch.fromDict(configs)
        # return Cross_Transformer(opt.enc_layers, opt.enc_rnn_size,
        #                           opt.heads, opt.transformer_ff,
        #                           opt.dropout, embeddings)
        return Cross_Transformer(configs, embeddings, embeddings1), TransformerEncoder(opt.enc_layers, opt.enc_rnn_size, opt.heads, opt.transformer_ff, opt.dropout, embeddings), TransformerEncoder(opt.enc_layers, opt.enc_rnn_size, opt.heads, opt.transformer_ff, opt.dropout, embeddings1)
...
def build_base_model(model_opt, fields, gpu, checkpoint=None):
    ...
    if model_opt.model_type == "text":
        ...
        contextual_transformer, encoder, encoder1 = build_encoder(model_opt, src_embeddings, src_embeddings1)
    ...
    model = onmt.models.NMTModel(contextual_transformer, encoder, encoder1, decoder, decoder1, decoder2)
    ...
    if checkpoint is not None:
        ...
        if hasattr(model.encoder1, 'embeddings'):
            model.encoder1.embeddings.load_pretrained_vectors(
                model_opt.pre_word_vecs_enc, model_opt.fix_word_vecs_enc)
        if hasattr(model.contextual_transformer, 'embeddings'):
            model.contextual_transformer.embeddings.load_pretrained_vectors(
                model_opt.pre_word_vecs_enc, model_opt.fix_word_vecs_enc)
