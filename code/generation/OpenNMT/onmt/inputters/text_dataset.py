...
class TextDataset(DatasetBase):
    ...
    def collapse_copy_scores(scores,
        ...
        for b in range(scores.size(batch_dim)):
            ...
            # print('index', index)
            # print('src_vocab', len(src_vocabs[0]))
            src_vocab = src_vocabs[index]
            for i in range(1, len(src_vocab)):
                sw = src_vocab.itos[i]
                ti = tgt_vocab.stoi[sw]
                if ti != 0 and ((offset+i) < scores.size()[1]):
    ...
    def make_examples(text_iter, truncate, side):
        ...
        """
        def fenci(str):  # 返回一个list
            # with open('/home/dell/zjx/MSParS_me/MSParS_V2.0/data/single-turn/task2/process/stop_words.txt', encoding='utf-8') as f: # 可根据需要打开停用词库，然后加上不想显示的词语
            #     con = f.readlines()
            ...
            jieba.load_userdict('/home/dell/zjx/MSParS_V2.0_single_2_newme/data/single-turn/task2/category_name.txt')
            result = []
            for word in jieba.lcut(str, cut_all=False):
                # if word not in stop_words:
                result.append(word)
            return result
    
        def fencistop(str):  # 返回一个list
            with open('/home/dell/zjx/MSParS_me/MSParS_V2.0/data/single-turn/task2/process/stop_words.txt', encoding='utf-8') as f: # 可根据需要打开停用词库，然后加上不想显示的词语
                con = f.readlines()
                stop_words = set() # 集合可以去重
                for i in con:
            ...
            jieba.load_userdict('/home/dell/zjx/MSParS_me/MSParS_V2.0/data/single-turn/task2/process/category_name.txt')
            result = []
            for word in jieba.lcut(str, cut_all=False):
                # if word not in stop_words:
                result.append(word)
        ...
        if side == "tgt" or side == 'tgt1' or side == 'tgt2':
            for i, line in enumerate(text_iter):
                ...
                # 分词
                seg_list = fenci(template[0])
                template = seg_list + template[1:]
                # print(template)
            ...
            for i, line in enumerate(text_iter):
                ...
                # 分词
                context = fenci(context[0])
                
                '''entity分词'''
                entity1_temp = line[1].split()
                entity1 = entity1_temp[:3] + fenci(entity1_temp[4])
                entity2_temp = line[2].split()
                entity2 = entity2_temp[:3] + fenci(entity2_temp[4])

                if truncate:
                    context = context[:truncate]
                    entity1 = " ".join(entity1[:truncate])
                    entity2 = " ".join(entity2[:truncate])
